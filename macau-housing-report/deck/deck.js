...
/* ================= 3 · 核心論點 ================= */
{
  ...
  const data = [
    ["−34.1", "%", "樓價由高峰回落", "2020 年第 4 季每平方米 108,969 元\n降至 2025 年第 1 季 71,859 元"],
    ["−71.3", "%", "成交量萎縮", "2019 年第 4 季 1,831 宗\n降至 2023 年第 4 季 525 宗"],
    ["+2.1", "%", "住宅租金逆勢上升", "2025 年全年每平方米實用面積\n平均租金按年升幅"],
    ["10.5", " 倍", "樓價收入比仍偏高", "雙職家庭推算；國際合理\n區間為 3 至 5 倍"],
  ];
...
function dataTable(s, { x, y, w, head, rows, colW, rowH, align, hi, cap }) {
  ...
  head.forEach((hd, i) => {
    s.addText(hd, {
      x: cx, y: cy, w: cw[i], h: 0.34, isTextBox: true, margin: 0,
  ...
  rows.forEach((r, ri) => {
    ...
    r.forEach((cell, ci) => {
      ...
      s.addText(String(cell), {
        x: rx, y: cy, w: cw[ci], h: rh - 0.08, isTextBox: true, margin: 0,
...

{
  ...
  hr(s, M, 5.45, CW, 0.02);
  body(s, "2024 年全年整體均價為 88,874 元，按年下跌 9.0%，但全年住宅買賣數目反而增加 17.4%。由高峰 108,969 元至 2025 年第 1 季 71,859 元，累計跌幅 34.1%。\n單位愈大，每平方米售價愈高（全澳由 67,826 升至 77,699 元）—— 這一點在下一組租金數據會出現關鍵反轉。", {
...

{
  ...
  ruledList(s, {
    ...
    items: [
      ...
      ["以 2024 全年均價 88,874 元計", "總價 6,221,180 元　→　13.0 倍"],
      ["以 2025 第 1 季 71,859 元計", "總價 5,030,130 元　→　10.5 倍"],
      ["單一收入者（2025 第 1 季基準）", "約 21.0 倍"],
      ["國際合理區間", "3 至 5 倍"],
    ],
    hi: [3],
  });
  bigStat(s, { x: 8.9, y: 2.5, w: 3.8, value: "10.5", unit: " 倍", label: "雙職家庭　2025 年第 1 季", note: "即使樓價由高峰回落三成，樓價收入比僅由 13.0 倍降至 10.5 倍，依然是國際合理上限的兩倍以上。單一收入者需約 21 年。", size: 52 });
  s.addNotes("計算式要在投影片上完整顯示，這是學術誠信的要求，也方便老師核對。強調結論：跌了三成，比率只由 13 降到 10.5，仍是上限兩倍以上。");
}

{
  ...
  label(s, "為何與本報告的 10.5 倍差距這麼大", { x: 6.9, y: 2.42, w: 5.4, size: 13 });
...

{
  ...
  label(s, "社屋存量的時間變化", { x: 8.9, y: 4.6, w: 3.8, size: 13 });
  body(s, "12,753 間（2016 年底，高武洲 2017）\n15,037 間（2021 年 6 月底）\n\n推算需求 17,394 間，缺口持續存在。", {
    x: 8.9, y: 5.05, w: 3.8, h: 1.4, size: 12.5,
  });
...
const factors = [
  {
    ...
    stats: [["17,000", " 元", "2025 年第 3 季月收入中位數"], ["70,935", " 元", "住宅每平方米均價（政府新聞稿）"], ["26.1", "%", "就業居民從事文娛博彩及服務業"]],
...

{
  ...
  ruledList(s, {
    ...
    items: [
      ...
      ["工具二　地租理論與土地壟斷", "樓價收入比約 10.5 倍　即十年勞動所得換取居住權"],
...
const tools = [
  ...
  },
  {
    ...
    evid: [["30.5", " 平方公里", "澳門總陸地面積"], ["2.9", " 平方公里", "商住及住宅用地"], ["10.5", " 倍", "雙職家庭樓價收入比"]],
...

{
  ...
  hr(s, 6.9, 3.35, 5.8);
  body(s, "夾屋是階梯房屋政策中唯一針對夾心階層的環節，但 2026 年政府評估其需求不算迫切，快環順利樓北側地塊的用途仍在檢討。建議明確該地塊的夾屋用途、公布單位數量與落成時間表，並維持以相鄰區域私樓價格折扣定價、設不可轉讓期限的設計。", {
    x: 6.9, y: 3.55, w: 5.8, h: 2.0, size: 12.5,
  });
  label(s, "避免公共資源外溢為投資收益", { x: 6.9, y: 5.62, w: 5.8, size: 11.5 });
...

{
  ...
  ruledList(s, {
    ...
    items: [
      ["2025 年全年樓價數字對不上帳", "政府新聞稿載每平方米 70,935 元、按年 −16.7%，但已核實的 2024 年全年為 88,874 元，兩者相除應為 −20.2%。本報告改用已核實的季度序列。"],
...

{
  ...
  hr(s, M, 4.6, 7.8, 0.02);
  body(s, "澳門樓價五年累計下跌三成，成交量萎縮七成，而租金逆勢上升、收入中位數下跌。同一時期，政府握有數千億財政儲備，建成的置換房有一千七百多個單位待售、兩千八百多個暫住房完全未啟用，而舊區仍有四千八百多幢樓齡三十年以上的樓宇等待更新。", {
  ...
  s.addNotes(
    ...
    "然後把四個關鍵數字再唸一次：−34.1%、−71.3%、+2.1%、10.5 倍。\n" +
